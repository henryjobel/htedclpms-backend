...
function buildCashFlowSeries(schedules, bills) {
    const monthMap = {};
    schedules.forEach((schedule) => {
        if (!schedule.paidDate)
            return;
...
router.get("/chart", auth_1.authenticate, async (_req, res) => {
    try {
...
router.post("/chart", auth_1.authenticate, async (req, res) => {
    try {
        const account = await prisma_1.prisma.account.create({ data: req.body });
...
router.put("/chart/:id", auth_1.authenticate, async (req, res) => {
    try {
        const accountId = req.params.id;
        const account = await prisma_1.prisma.account.update({ where: { id: accountId }, data: req.body });
...
router.delete("/chart/:id", auth_1.authenticate, async (req, res) => {
    try {
        const accountId = req.params.id;
        await prisma_1.prisma.account.update({ where: { id: accountId }, data: { isActive: false } });
...
router.get("/vouchers", auth_1.authenticate, async (req, res) => {
    try {
        ...
        if (startDate && endDate) {
            where.voucherDate = { gte: new Date(startDate), lte: new Date(endDate) };
        }
...
router.patch("/vouchers/:id/approve", auth_1.authenticate, async (req, res) => {
    try {
...
router.get("/approval-logs", auth_1.authenticate, async (req, res) => {
    try {
        ...
        const rows = await prisma_1.prisma.approvalLog.findMany({
            where,
            include: {
...
router.get("/ledger", auth_1.authenticate, async (req, res) => {
    try {
...
router.get("/installments", auth_1.authenticate, async (_req, res) => {
    try {
...
router.post("/installments", auth_1.authenticate, async (req, res) => {
    try {
...
router.post("/installments/:id/pay", auth_1.authenticate, async (req, res) => {
    try {
...
router.get("/bank-accounts", auth_1.authenticate, async (_req, res) => {
    try {
...
router.get("/cash-book", auth_1.authenticate, async (req, res) => {
    try {
...
router.post("/bank-accounts", auth_1.authenticate, async (req, res) => {
    try {
...
router.get("/bank-reconciliations", auth_1.authenticate, async (_req, res) => {
    try {
        const rows = await prisma_1.prisma.bankReconciliation.findMany({
            include: {
...
router.post("/bank-reconciliations", auth_1.authenticate, async (req, res) => {
    try {
        const bookBalance = Number(req.body.bookBalance || 0);
        const statementBalance = Number(req.body.statementBalance || 0);
        const item = await prisma_1.prisma.bankReconciliation.create({
            data: {
                ...req.body,
...
router.put("/bank-reconciliations/:id", auth_1.authenticate, async (req, res) => {
    try {
        const bookBalance = Number(req.body.bookBalance || 0);
        const statementBalance = Number(req.body.statementBalance || 0);
        const item = await prisma_1.prisma.bankReconciliation.update({
            where: { id: req.params.id },
            data: {
                ...req.body,
...
router.delete("/bank-reconciliations/:id", auth_1.authenticate, async (req, res) => {
    try {
        await prisma_1.prisma.bankReconciliation.delete({ where: { id: req.params.id } });
...
router.get("/cheques", auth_1.authenticate, async (_req, res) => {
    try {
        const rows = await prisma_1.prisma.chequeEntry.findMany({
            include: {
                bankAccount: { select: { name: true, bankName: true, accountNo: true } },
            },
...
router.post("/cheques", auth_1.authenticate, async (req, res) => {
    try {
        const item = await prisma_1.prisma.chequeEntry.create({
            data: {
                ...req.body,
...
router.put("/cheques/:id", auth_1.authenticate, async (req, res) => {
    try {
        const item = await prisma_1.prisma.chequeEntry.update({
            where: { id: req.params.id },
            data: {
                ...req.body,
                amount: Number(req.body.amount || 0),
...
router.delete("/cheques/:id", auth_1.authenticate, async (req, res) => {
    try {
        await prisma_1.prisma.chequeEntry.delete({ where: { id: req.params.id } });
...
router.get("/balance-sheet", auth_1.authenticate, async (_req, res) => {
    try {
        const [accounts, bankAccounts, installments, bills] = await Promise.all([
            prisma_1.prisma.account.findMany({ where: { isActive: true } }),
            prisma_1.prisma.bankAccount.findMany({ where: { isActive: true } }),
            prisma_1.prisma.installment.findMany(),
...
router.get("/cash-flow", auth_1.authenticate, async (_req, res) => {
    try {
        const [installments, bills] = await Promise.all([
            prisma_1.prisma.installment.findMany({ include: { schedule: true } }),
            prisma_1.prisma.bill.findMany(),
        ]);
        const cashFlow = buildCashFlowSeries(installments.flatMap((inst) => inst.schedule), bills);
...
router.get("/dashboard-summary", auth_1.authenticate, async (_req, res) => {
    try {
        const [projects, installments, bills, products, requisitions, rfqs, purchaseOrders] = await Promise.all([
            prisma_1.prisma.project.findMany({ include: { projectExpenses: true, installments: true } }),
            prisma_1.prisma.installment.findMany({ include: { schedule: true } }),
            prisma_1.prisma.bill.findMany({ where: { status: "unpaid" } }),
            prisma_1.prisma.product.findMany({ where: { isActive: true } }),
            prisma_1.prisma.materialRequisition.count({ where: { status: "PENDING" } }),
            prisma_1.prisma.rFQ.count({ where: { status: "PENDING" } }),
            prisma_1.prisma.purchaseOrder.count({ where: { status: { in: ["DRAFT", "SENT"] } } }),
        ]);
        const paidBills = await prisma_1.prisma.bill.findMany({ where: { status: "paid" } });
        ...
        const cashFlow = buildCashFlowSeries(installments.flatMap((inst) => inst.schedule), paidBills)
...
router.get("/sales-forecast", auth_1.authenticate, async (req, res) => {
    try {
        const months = Math.min(parseInt(req.query.months || "6", 10) || 6, 12);
        const now = new Date();
        ...
        installments.forEach((installment) => {
            ...
            installment.schedule.forEach((schedule) => {
                ...
                if (dueDate < now) {
                    ...
                    if (!forecastMap[currentKey])
                        forecastMap[currentKey] = { expected: 0, overdue: 0 };
                ...
                if (!forecastMap[key])
                    forecastMap[key] = { expected: 0, overdue: 0 };
        ...
        const projectForecast = Object.values(projectMap)
            .map((project) => ({
            ...project,
            collectionRate: project.totalReceivable + project.nextExpected > 0
...
router.get("/trial-balance", auth_1.authenticate, async (_req, res) => {
    try {
        const accounts = await prisma_1.prisma.account.findMany({
            where: { isActive: true },
            include: { ledgerEntries: true },
...
router.get("/day-book", auth_1.authenticate, async (req, res) => {
    try {
...
router.get("/receive-payment-summary", auth_1.authenticate, async (_req, res) => {
    try {
        const vouchers = await prisma_1.prisma.voucher.findMany({
...
router.get("/pending-approvals", auth_1.authenticate, async (_req, res) => {
    try {
        const [vouchers, requisitions, purchaseOrders, billings] = await Promise.all([
            ...
            prisma_1.prisma.materialRequisition.findMany({
                where: { status: "PENDING" },
                include: { project: { select: { name: true } } },
                orderBy: { createdAt: "desc" },
            }),
            ...
            prisma_1.prisma.billingRecord.findMany({
